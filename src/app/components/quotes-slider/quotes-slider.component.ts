...
export class QuotesSliderComponent {
  ...
  nextQuote() {
    this.sliderQuoteIndex = (this.sliderQuoteIndex + 1) > this.quotes.length - 1 ? 0 : this.sliderQuoteIndex + 1;
  }

  prevQuote() {
    this.sliderQuoteIndex = (this.sliderQuoteIndex - 1) < 0 ? this.quotes.length - 1 : this.sliderQuoteIndex - 1;
  }
}
